...
<html>
  ...
  <source src="Paytm - Notification.mp3" type="audio/mpeg">
...
<ul class="nav navbar-nav navbar-right">
            <li><a href="#" class="dropdown-toggle active" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-user"></span> Sign Up <span class="caret"></span> </a>
              ...
              <li> <a href="#"> manager Sign-up</a></li>
            </ul>
            </li>
...
<h2 class="text-center"> Thank you for shopping at College Canteen! The ordering process is now complete.</h2>
...
if (isset($_GET['order_IDs'])) {
  ...
  // Handle the case when no order IDs are passed
  //echo "No order IDs found.";
  exit;
}

// Update the payment status for each order ID
$payment_status = "paid";
...
foreach ($order_IDs as $order_ID) {
  ...
  // Update the payment status for the current order ID
  $query = "UPDATE orders SET payment_status = '$payment_status' WHERE order_ID = '$order_ID'";
  $result = mysqli_query($conn, $query);

  if ($result) {
      // Payment status updated successfully for the current order ID
      //echo "Payment status updated successfully for Order ID: " . $order_ID . "<br>";
  } else {
      // Error occurred while updating payment status for the current order ID
      echo "Error updating payment status for Order ID: " . $order_ID . "<br>";
  }
}
...
?>
<audio id="orderSound" autoplay>
  <source src="order_sound.mp3" type="audio/mpeg">
  Your browser does not support the audio element.
</audio>
